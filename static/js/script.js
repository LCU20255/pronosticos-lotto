document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTags() {
        ...
        pastAnimalsList.forEach((animal, index) => {
            ...
            tag.className = 'flex items-center gap-1.5 px-3 py-1.5 rounded-lg border-2 border-transparent text-xs font-bold animate-fade-in transition-all hover:bg-white hover:border-brand-prime/20 hover:shadow-subtle hover:-translate-y-0.5 cursor-default';
            tag.innerHTML = `
                <span>${animal}</span>
                <button type="button" class="text-brand-prime/50 hover:text-red-500 transition-colors ml-1 w-5 h-5 flex items-center justify-center rounded-full hover:bg-red-50" data-index="${index}">
    ...
    const fetchLiveFeed = async () => {
        ...
        try {
            ...
            if (!data || Object.keys(data).length === 0) {
                feedList.innerHTML = `<div class="col-span-full p-10 text-center text-sm text-slate-500 bg-white/50 backdrop-blur-xl rounded-2xl border border-white shadow-subtle font-medium">No official datastream present on node yet.</div>`;
                return;
            }
            ...
            for (const [lotteryName, results] of Object.entries(data)) {
                htmlAccumulator += `
                <div class="card-float overflow-hidden flex flex-col h-full hover:-translate-y-1 transition-transform group/card border-transparent">
                    <div class="bg-gradient-to-r from-slate-50 to-white text-slate-800 text-xs font-extrabold uppercase tracking-widest py-4 px-5 flex items-center justify-between border-b border-slate-100/50">
                        <span class="flex items-center gap-2"><i class="fa-solid fa-bolt text-brand-prime group-hover/card:animate-pulse"></i> ${lotteryName}</span>
                    </div>
                    <div class="divide-y divide-slate-100/30 flex-grow bg-white/40">
                `;
                
                if (!results || results.length === 0) {
                    htmlAccumulator += `<div class="p-8 text-center text-xs text-slate-400 font-bold uppercase tracking-widest">No datastream</div>`;
                } else {
                    results.forEach((item, index) => {
                        const delay = index * 40; // Elegant cascade
                        htmlAccumulator += `
                        <div class="p-4 px-5 flex items-center justify-between animate-[fadeIn_0.5s_ease-out_forwards] hover:bg-white/80 transition-all group/item" style="animation-delay: ${delay}ms; opacity: 0;">
                            <span class="text-slate-400 font-semibold text-xs w-20 group-hover/item:text-brand-prime transition-colors">${item.schedule.replace(/ AM| PM/g, '')} <span class="text-[0.6rem] opacity-70 font-bold">${item.schedule.slice(-2)}</span></span>
                            <div class="flex items-center gap-3 flex-grow justify-end">
                                <span class="text-2xl font-black text-slate-900 w-8 text-center tracking-tighter mix-blend-multiply">${item.animal.split(' ')[0]}</span>
                                <span class="text-[0.65rem] font-bold text-brand-prime tracking-widest uppercase bg-brand-prime/5 px-2 py-1 rounded-md group-hover/item:bg-brand-prime/10 transition-colors">${item.animal.substring(item.animal.indexOf(' ') + 1)}</span>
                            </div>
                        </div>
                        `;
                    });
    ...
    if (btnGenerateAI) {
        btnGenerateAI.addEventListener('click', async () => {
            ...
            setTimeout(() => {
                 ...
                 // Meta DOM references
                 const metaBar = document.getElementById('ai-metadata-bar');
            ...
            try {
                 ...
                 if(data.analysis_meta) {
                     const metaBar = document.getElementById('ai-metadata-bar');
                     const metaQuad = document.getElementById('ui-meta-quadrant');
                     const metaMirror = document.getElementById('ui-meta-mirror');
                     
                     if(metaBar && metaQuad && metaMirror) {
                         metaBar.classList.remove('hidden');
                         metaQuad.textContent = data.analysis_meta.dominant_quadrant || 'Mixto';
                         metaMirror.textContent = data.analysis_meta.hot_mirror || 'Ninguno';
                     }
                 }
                 ...
                 // Helper to generate spectacular chips inside a card
                 const generateChips = (combinations) => {
                     return combinations.map(c => `
                         <div class="flex items-center gap-3 bg-white/80 backdrop-blur-sm border border-slate-100 rounded-xl px-5 py-3 mb-2 shadow-sm transition-all hover:shadow-elegant hover:scale-[1.02] hover:border-brand-prime/30 group/chip cursor-default">
                            <span class="text-3xl font-black text-slate-900 tracking-tighter drop-shadow-sm">${c.number}</span>
                            <span class="text-xs font-extrabold text-brand-prime tracking-widest uppercase bg-brand-prime/5 px-2 py-0.5 rounded-md">${c.animal}</span>
                         </div>
                     `).join('');
                 };
                 ...
                 if(data.primary_play) {
                     ...
                     htmlCards += `
                        <div class="md:col-span-1 ai-card-primary p-7 flex flex-col items-center justify-start text-center relative hover:scale-[1.01] transition-transform">
                            <span class="text-xs font-bold text-brand-prime uppercase tracking-widest mb-5 flex items-center gap-1.5"><i class="fa-solid fa-sparkles text-amber-400 drop-shadow-md"></i> Prime Target</span>
                            ...
                            <div class="mt-8 w-full bg-slate-100 rounded-full h-2.5 overflow-hidden shadow-inner">
                                <div class="bg-gradient-to-r from-brand-prime to-[#818cf8] h-full rounded-full transition-all duration-1000 relative" style="width: ${probFloat}%">
                                </div>
                            </div>
                            <span class="text-[0.65rem] text-slate-500 font-bold mt-3 block uppercase tracking-widest w-full text-right">${data.primary_play.probability || '0%'} CONFIDENCE</span>
                        </div>
                     `;
                 }
                 ...
                 if(data.secondary_plays && Array.isArray(data.secondary_plays)) {
                     const secondaryGrid = data.secondary_plays.map((sp, idx) => {
                         ...
                         return `
                         <div class="ai-card-secondary p-6 flex flex-col items-center justify-start text-center group/sec relative">
                            <span class="text-[0.65rem] font-bold text-slate-400 tracking-widest uppercase mb-5 transition-colors group-hover/sec:text-brand-prime">Node Backup ${idx + 1}</span>
                            
                            <div class="w-full flex inset-wrap flex-col gap-1 items-center justify-center flex-grow opacity-90 group-hover/sec:opacity-100 transition-opacity">
                                ${generateChips(sp.combination || [])}
                            </div>

                            <div class="mt-auto pt-7 w-full">
                                <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden shadow-inner">
                                    <div class="bg-slate-300 h-full rounded-full transition-all duration-1000 group-hover/sec:bg-brand-prime" style="width: ${probFloat}%"></div>
                                </div>
                                <span class="text-[0.65rem] text-slate-400 font-bold mt-2.5 block uppercase tracking-widest group-hover/sec:text-slate-600 transition-colors">${sp.probability || '0%'}</span>
                            </div>
                        </div>
                         `;
                     }).join('');
